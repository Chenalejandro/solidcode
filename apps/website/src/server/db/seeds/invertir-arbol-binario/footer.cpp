...
vector<long> treeAsArray;

vector<long> treeToArray(Nodo *raiz) {
  treeAsArray.clear();
  ...
  queue.push(raiz);
  while (!queue.empty()) {
    Nodo *currentNode = queue.front();
    queue.pop();
    if (currentNode == nullptr) {
      treeAsArray.push_back(-1);
      continue;
    }
    treeAsArray.push_back(currentNode->valor);
    queue.push(currentNode->izquierdo);
    queue.push(currentNode->derecho);
  }

  while (treeAsArray.size() != 0 && treeAsArray[treeAsArray.size() - 1] == -1) {
    treeAsArray.pop_back();
  }
  return treeAsArray;
}
...
int main() {
  ...
  for (auto i = 0; i < input_count; i++) {
    vector<long> result =
        treeToArray(invertirArbolBinario(convertArrayToTree(arrays[i])));
    stringstream ss;
