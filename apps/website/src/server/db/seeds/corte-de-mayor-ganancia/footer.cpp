...
int main() {
  ...
  vector<int> longitudes(input_count);

  for (auto i = 0; i < input_count; i++) {
    string longitud_temp;
    getline(cin, longitud_temp);

    int longitud = stoi(ltrim(rtrim(longitud_temp)));
  ...
  for (auto i = 0; i < input_count; i++) {
    long result = corteDeMayorGanancia(arrays[i], longitudes[i]);
    if (i != input_count - 1) {
      fout << result << "\n";
    } else {
      fout << result;
